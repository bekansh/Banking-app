...
con=pymysql.connect(host='localhost', user='root', password='mysql', db='ATM')
c=con.cursor()

c.execute('create table if not exists details(name varchar(500), password varchar(500), balance float)')
...
def old_user():
    ...
    def submit2():
        ...
        if(password1.get()==''):
            ...
            if flag == 0:
                name_error.config(text='*Username does not exists')
...
#Deposit
    ...
    def deposit(balance):
        ...
        def func2(balance):
            ...
            elif(float(money2.get())>10000000):
                deposit_error.config(text='*Our bank allows a maximum deposition of Rs.1 crore in a go')
